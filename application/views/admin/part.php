...
<button style="float: right; margin-right: 10px;" class="btn btn-danger btn-md" data-toggle="modal" onclick="add_gmc();">Delete</button> -->
        ...
        <button class="btn btn-success" onclick="add_part()"><i class="glyphicon glyphicon-plus"></i> Add Part</button>
                    ...
                    <div class="form-body">
                         <div class="modal-body mx-3">
                            <div class="md-form mb-5">
                                <label data-error="wrong" data-success="right" for="item_part">Part No</label>
                                <input type="text" name="item_part" id="item_part" class="form-control validate">                                
                            </div>

                            <div class="md-form mb-5">
                                <label data-error="wrong" data-success="right" for="nama_part">Part Name</label>
                                <input type="text" name="nama_part" id="nama_part" class="form-control validate">
                            </div>

                            <div class="md-form mb-5">
                                <label data-error="wrong" data-success="right" for="std_price">Price</label>
                                <input type="text" name="std_price" id="std_price" class="form-control validate">
                            </div>

                            <div class="md-form mb-5">
                                <label data-error="wrong" data-success="right" for="qty_use">Using</label>
                                <input type="text" name="qty_use" id="qty_use" class="form-control validate">
                            </div>

                            <div class="md-form mb-5">
                                <label data-error="wrong" data-success="right" for="satuan">Satuan</label>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('part/ajax_list')?>",
            "type": "POST"
        },
...
function add_part()
{
    ...
    $('.modal-title').text('Add PART'); // Set Title to Bootstrap modal title
...
function edit_value(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('part/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('part/ajax_add')?>";
    } else {
        url = "<?php echo site_url('part/ajax_update')?>";
    }
...
function delete_value(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('part/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "<?php echo site_url('part/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
